...
export function exportStoryboardToMarkdown(response: StoryboardResponse) {
  ...
  return [
    ...
    `Retrieval mode: ${response.retrieval.mode}`,
    `Retriever: ${response.retrieval.provider} (${response.retrieval.model})`,
    `Generation mode: ${response.generation.mode}`,
    `Generator: ${response.generation.provider} (${response.generation.model})`,
    `Evaluation mode: ${response.evaluationMeta.mode}`,
    `Evaluator: ${response.evaluationMeta.provider} (${response.evaluationMeta.model})`,
    ...
    ...(response.retrieval.warning ? [`Retriever warning: ${response.retrieval.warning}`] : []),
    ...(response.generation.warning ? [`Generator warning: ${response.generation.warning}`] : []),
    ...(response.evaluationMeta.warning ? [`Evaluator warning: ${response.evaluationMeta.warning}`] : []),
    "",
    `## Scene goal`,
